...
#' @param pars \code{\link{vector}} with 4 values: the death rate, the mean transmission rate, the amplitude of sinusoidal forcing, the frequency of oscillations and the recovery rate. The names for these values must be: "mu", "beta0", "beta1", "omega" and "gamma", respectively.
...
#' parameters <- c(beta0 = 17 / 13, beta1 = 0.1,
#'                 gamma = 1 / 13, omega = pi,
#'                 mu = 1 / (50 * 365))
#' 
  ...
  function1 <- function(pars = NULL, init = NULL, time = NULL) {
    function2 <- function(time, init, pars) {
      with(as.list(c(init, pars)), {
        beta <- beta0 * (1 + beta1 * sin(2 * omega * time / 365))
        dS <- mu - beta * S * I - mu * S
