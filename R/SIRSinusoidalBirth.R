...
#' @details This is the R version of program 5.3 from page 184 of "Modeling Infectious Disease in humans and animals" by Keeling & Rohani.
...
SIRSinusoidalBirth <- function(pars = NULL, init = NULL, time = NULL, ...) {
  ...
  if (length(pars$alpha1) == 1){
    ...
    Bifur_Infectious <- matrix(0,nrow=alpha1.range,ncol=10)
    
    for (i in 1:alpha1.range){
      
      pars.solve.sirsb$alpha1 <- alpha2[i]
      solve.result <- SIRSinusoidalBirth(pars = pars.solve.sirsb, init = init.solve.sirsb, time = time)
      
      init.solve.sirsb  <- c(S = as.numeric(tail(solve.result$results, 1)[2]),
                             I = as.numeric(tail(solve.result$results, 1)[3]), 
                             R = as.numeric(tail(solve.result$results, 1)[4]))
      
      for (j in 0:9){
        Bifur_Infectious[i,j+1]  <-  solve.result$results[seq(1:max(time))[((max(time) - j * 365) - 1)], 'I']
      }
      
    }
    
    if(min(pars$alpha1)>=0 & max(pars$alpha1)<=1){
      plot(alpha2, log10(t(Bifur_Infectious)[1,]),ylim = c(-3.194,-3.17),xlim = c(0, 1),
           xlab = 'Seasonality in Births, alpha1', ylab = 'log10(Proportion Infectious, I)',
           pch = 16, cex = 0.5)
      for (i in 2:10){
        points(alpha2, log10(t(Bifur_Infectious)[i,]),
               pch = 16, cex = 0.5)
      }  
    }      
    
    return(Bifur_Infectious)
  }
